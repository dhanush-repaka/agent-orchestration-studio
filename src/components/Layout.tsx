...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const [headerMenu, setHeaderMenu] = useState<'notifications' | 'help' | 'user' | null>(null);
        ...
        <div className="flex items-center gap-1 ml-auto md:ml-2 relative">
          ...
          <div className="relative ml-1 pl-2 border-l border-slate-200 dark:border-slate-700">
            <button
              ...
              onClick={() => setHeaderMenu((m) => (m === 'user' ? null : 'user'))}
            >
            ...
            {headerMenu === 'user' && (
              <div className="absolute right-0 top-11 w-64 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg z-50 p-2">
                <div className="px-2 py-2 border-b border-slate-100 dark:border-slate-800 mb-1">
                  ...
                  <p className="text-[11px] text-slate-400 mt-0.5">{currentUser.role}</p>
                </div>
                <button
                  type="button"
                  className="w-full flex items-center gap-2 px-2 py-2 rounded-lg text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800"
                  onClick={() => { setHeaderMenu(null); void signOut(); }}
                >
