...
import { callEdgeFunction, type AgentProcessorPayload, type AgentProcessorResult } from '@/lib/api';
...
async function runAgentNode(
  ...
  if (agent.type === 'Data Retrieval') {
    ...
    const { ok, data } = await callEdgeFunction<Record<string, unknown>>('ado-retrieval', {
  ...
  if (agent.type === 'ADO Upload') {
    ...
    const { ok, data } = await callEdgeFunction<Record<string, unknown>>('ado-upload', {
      testCases,
      sourceWorkItemId,
      adoOrg: cfg.adoOrg || undefined,
  ...
  const { ok, data } = await callEdgeFunction<AgentProcessorResult>('agent-processor', payload as unknown as Record<string, unknown>);
  if (!ok || data.error) {
...
export async function executeWorkflow(opts: {
  ...
  const { workflow: wf, agents, callbacks } = opts;
        ...
        default: {
          if (node.data.kind === 'agent') {
            ...
            for (let attempt = 0; attempt <= retries; attempt++) {
              last = await runAgentNode(node, agent, wf, workflowInput, nodeOutputs, nodes, agentMap, (l, s, m) => addLog(l, s, m, currentId));
              if (!last.error) break;
